...
import views.Trending;
import views.Index;
import views.Login;
import views.Languages;
import views.Search;
import views.DisplayPost;
...
public class Main 
{
	...
	{		
		...
		registerShutdownHook(databaseInterface);		System.out.println("Project:csc15");

		Profile iProfile = new Profile(databaseInterface,fileStoreInterface);

		Trending iTrending = new Trending(databaseInterface,fileStoreInterface);

		Index iIndex = new Index(databaseInterface,fileStoreInterface);

		Login iLogin = new Login(databaseInterface,fileStoreInterface);

		Languages iLanguages = new Languages(databaseInterface,fileStoreInterface);

		Search iSearch = new Search(databaseInterface,fileStoreInterface);

		DisplayPost iDisplayPost = new DisplayPost(databaseInterface,fileStoreInterface);
		...
		while(shouldKeepRunning)
		{
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
				...
				if(iProfile.process(toProcess))
				{
				}
				else
					if(iTrending.process(toProcess))
					{
					}
					else
						if(iIndex.process(toProcess))
						{
						}
						else
							if(iLogin.process(toProcess))
							{
							}
							else
								if(iLanguages.process(toProcess))
								{
								}
								else
									if(iSearch.process(toProcess))
									{
									}
									else
										if(iDisplayPost.process(toProcess))
										{
										}
										else
										{
